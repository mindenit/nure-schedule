...
import { ITimestampTransformer } from "../interfaces/index.js";
...
export class EventsBuilder<T> implements TEventsBuilder<T> {
    ...
    addEvents(day: IDay, events: TRawEvent<T>[]): TDayWithEvents<T> {
        return {
            ...day,
            events: this.#getDayEvents(day.day, day.month, events),
        };
    }
    ...
    #getDayEvents(
        ...
        const dayEvents = events.filter((event) => {
            ...
            const eventMonth = this.timestampTransformer.toMonth(
                event.startTime
            );
        ...
        const trasformedEvents =
            this.eventsTransformer.transformEvents(dayEvents);
