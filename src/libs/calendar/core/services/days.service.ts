...
import { TDaysService, TEventsBuilder, TServiceConfig } from "../types/index.js";
...
export class DaysService<T> implements TDaysService<T> {
	...
	public getDay(events: TRawEvent<T>[]) {
		const { date: { date } } = this.config
		const day = this.daysTransformer.transformDay(date);

		const dayWithEvents = this.eventsBuilder.addEvents([day], events)
